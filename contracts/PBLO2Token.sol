...
contract PBLO2Token is BEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    // If this is included at any point in the transfer, then a fee should be included
    mapping (address => bool) private _isIncludedInFees;

    // If the address exists in this array, then there should never be any taxes, even if doing transaction with
    // an address in _isIncludedInFees
    ...
    // criteria for automatic burns from burn wallet
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount) private {
        ...
        //indicates if fee should be deducted from transfer. By default there is no tax
        bool takeFee = false;

        // check and see if any of the addresses are forced to include fees, if so, then turn on the fees
        if(
            isIncludedInFees(sender) ||
            isIncludedInFees(recipient)
        ) {
            takeFee = true;
        }

        //if any account belongs to _isExcludedFromFee account then remove the fee regardless
        if(
    ...
    function _transferWithTaxable(address sender, address recipient, uint256 tAmount) private {
        // get all of the values for the fees
        (uint256 tTransferAmount, uint256 tBurn, uint256 tMarketing, uint256 tCharity) = _getTValues(tAmount);
    ...
    // get all of tax values
    function _getTValues(uint256 tAmount) private view returns(uint256 tTransferAmount, uint256 tBurn, uint256 tMarketing, uint256 tCharity) {
        uint256 tBurn = calculateBurnFee(tAmount);
        uint256 tMarketing = calculateMarketingFee(tAmount);
        uint256 tCharity = calculateCharityFee(tAmount);

        uint256 tTransferAmount = tAmount.sub(tBurn.add(tMarketing).add(tCharity));

        return (tTransferAmount, tBurn, tMarketing, tCharity);
    }

    // FUNCTIONS for owner to manage the lists of whom is included and excluded from fees
